      ...
      <main className="mt-8 space-y-8">
        ...
        <div className="rounded-lg bg-white dark:bg-slate-800 p-6 shadow-md dark:border dark:border-slate-700">
            ...
            <div className="space-y-4">
                <input type="text" placeholder="Twilio Account SID" value={currentTwilioConfig.accountSid} onChange={(e) => setCurrentTwilioConfig(prev => ({...prev, accountSid: e.target.value}))} className="w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 py-2 px-4 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
                <input type="password" placeholder="Twilio Auth Token" value={currentTwilioConfig.authToken} onChange={(e) => setCurrentTwilioConfig(prev => ({...prev, authToken: e.target.value}))} className="w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 py-2 px-4 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
                <input type="tel" placeholder="Your Twilio Phone Number" value={currentTwilioConfig.twilioPhoneNumber} onChange={(e) => setCurrentTwilioConfig(prev => ({...prev, twilioPhoneNumber: e.target.value}))} className="w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 py-2 px-4 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
            </div>
             <div className="mt-4 flex items-start gap-3 rounded-lg bg-amber-50 dark:bg-amber-900/20 p-3 text-amber-800 dark:text-amber-300">
                <ExclamationTriangleIcon className="h-5 w-5 flex-shrink-0 mt-0.5" />
                <div className="text-xs">
                    <p className="font-bold">Action Required for Live Features:</p>
                    <p>
                        For this app's login and calling to work when deployed, you MUST set your credentials as <strong>Environment Variables</strong> in your hosting provider's settings (e.g., Vercel). The backend functions will use these secure variables.
                    </p>
                    <p className="mt-2 font-semibold">Required Variables:</p>
                    <ul className="list-disc pl-4 mt-1">
                        <li><code className="text-xs">ADMIN_PASSWORD</code> (for admin login)</li>
                        <li><code className="text-xs">STAFF_PASSWORD</code> (for staff login)</li>
