...
import type { Student, TwilioConfig } from '../types';
...
type CallStatus = 'idle' | 'calling' | 'initiated' | 'completed' | 'error' | 'deployment-required';
interface CallState {
...
const apiCallToBackend = async (
    ...
    const timeoutId = setTimeout(() => {
        console.log('Backend call timed out after 5 seconds.');
        controller.abort();
    }, 5000);
    ...
    try {
        ...
        if (networkError.name === 'AbortError') {
            ...
            throw new BackendError('The request to the call service timed out. This usually means the backend is not running. Deploy the project to activate it.', true);
        }
...
const Dialer: React.FC<DialerProps> = ({ students, staffPhoneNumber, twilioConfig, addCallToHistory, markCallAsComplete, isRecordingEnabled }) => {
  ...
  const initiateCall = async (studentToCall: Student) => {
    ...
    // --- SAFETY NET TIMEOUT ---
    // This guarantees the user is never stuck on the "Connecting..." screen.
    // If the call hasn't succeeded or failed within 6 seconds, we assume the
    // backend is unavailable (e.g., in a local preview environment).
    const safetyTimeout = setTimeout(() => {
        setCallState(currentState => {
            if (currentState.status === 'calling') {
                console.warn('Safety timeout triggered. Forcing "deployment-required" state.');
                return {
                    student: studentToCall,
                    status: 'deployment-required',
                    message: 'The call service is not responding. This is expected in a local preview. Please deploy the project to enable live calls.'
                };
            }
            return currentState; // Don't change the state if it has already progressed
        });
    }, 6000);
    ...
    try {
        ...
        clearTimeout(safetyTimeout); // Success, cancel the safety net.
        
        setCallState(prev => ({ ...prev, status: 'initiated', callSid: data.callSid, message: '' }));
        addCallToHistory(studentToCall, data.callSid);

    } catch (error) {
        clearTimeout(safetyTimeout); // Failed with an error, cancel the safety net.
